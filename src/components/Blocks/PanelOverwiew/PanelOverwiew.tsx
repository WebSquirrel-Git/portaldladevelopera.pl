...
<h1 className='text-center text-white'>Twój Panel do legalnej<br/>
Sprzedaży <b className='gradient-text font-normal bg-gradientOrangeButton bg-white/40 bg-clip-text text-transparent'>Lokali</b>
</h1>
<div className='flex flex-col gap-8 items-center justify-center pb-0 2xl:pb-10'>
    <p className='text-white text-center'>{description}</p>
    <Link className='text-black rounded-lg box-border bg-primaryOrange border-[1px] border-white font-medium text-[18px] py-[18px] px-6 cursor-pointer' href={button.url}>{button.title}</Link>
</div>
            </div>
...
<Image
  ...
  />
                        ...
                        <p className='text-center gradient-text bg-gradientGreyWhite bg-clip-text text-transparent'>{testimonial.testimonial}</p>
                    </div>
                ))}
            </div>
