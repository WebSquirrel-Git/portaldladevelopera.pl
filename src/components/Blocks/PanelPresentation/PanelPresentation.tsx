...
{slidesArray.map((slide,i)=>(
    ...
    <div key={i} className={`flex flex-col-reverse gap-8 justify-between ${i%2===0?'xl:flex-row':'xl:flex-row-reverse'}`}>
       
        <div className='flex flex-col gap-8 max-w-[472px] justify-center'>
             <div className='xl:flex hidden'>
            <RichText data={slide.slide.header} className='richtext-h2-with-gradient !text-white '/>
        </div>
...
{slide.slide.slideList.map((listItem,z)=>(
    <div key={z} className='flex flex-row gap-2 items-center justify-start'>
        <Image src={StarIcon} className='w-6 h-6' alt='Gwiazdka'/>
         <RichText data={listItem.slideListDescription} className='richtext-p-with-link'/>
    </div>
))}
        </div>
        </div>
        <Media resource={slide.slide.image} className='xl:h-full xl:w-auto w-full h-auto' imgClassName='object-contain w-full'/>
         <div className='xl:hidden flex'>
        <RichText data={slide.slide.header} className='richtext-h2-with-gradient !text-white mx-0'/>
        </div>
    </div>
))}
         </div>
