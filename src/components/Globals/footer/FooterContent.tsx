...
export const FooterContent: React.FC<FooterContentProps> = ({
  ...
  return (
    <div className="flex flex-col w-full gap-8 2xl:gap-[120px]">
        ...
        <div className="flex flex-col sm:gap-1 gap-8 sm:flex-row justify-between w-full">
          ...
          <div className="flex flex-col gap-2 pl-[20px] xl:pl-0 justify-start items-start">
            ...
            <Link
              ...
              href={`mailto:${companyInformations.email ? companyInformations.email : ''}`}
            >
              {companyInformations.email ? companyInformations.email : ''}
            </Link>
            <Link
              className="text-[16px] text-white"
              href={companyInformations.website.url ? companyInformations.website.url : ''}
            >
              {companyInformations.website.label ? companyInformations.website.label : ''}
            </Link>
          </div>
          <div className="flex flex-col gap-2 pl-[20px] xl:pl-0 justify-start items-start">
            <span className="text-[14px] text-grey">Firma</span>
          ...
          <div className="flex flex-row gap-4">
            {companyInformations.facebook && (
              ...
              >
                <Icon
                  ...
                  icon={companyInformations.facebook.ikona || ''}
                />
              </Link>
            )}
            {companyInformations.instagram && (
              ...
              >
                <Icon
                  className="text-white w-6 h-6"
                  icon={companyInformations.instagram.ikona || ''}
                />
              </Link>
            )}
            {companyInformations.youtube && (
              ...
              >
                <Icon
                  className="text-white w-6 h-6"
                  icon={companyInformations.youtube.ikona || ''}
                />
              </Link>
            )}
          </div>
