...
export const HeaderClient: React.FC<HeaderProps> = ({ data }) => {
  ...
  return (
    <header className="w-full flex fixed top-0 left-0 flex-row box-border justify-between items-center h-fit z-20 py-[14px] 2xl:py-[20px] px-4 2xl:px-[200px] xl:px-[100px] bg-black">
      <Link
        className="flex flex-col xl:flex-row gap-1 xl:gap-5 xl:items-center items-start"
        href="/"
      >
        <Image src={Logo} alt="Piotr Wilk Meble" className="h-10 w-auto" />
        <span className="gradient-text bg-gradientGreyWhite bg-clip-text text-transparent xl:text-[24px] text-[10px]">
          PanelDlaDewelopera.pl
        </span>
      </Link>
      <NavListDesktop data={data} />
      <div
        className={`h-8 w-fit flex items-center pl-[1px] bg-grey xl:hidden ${showMenu ? 'hidden' : 'flex'}`}
      >
        <button
          role="button"
          aria-label="navigate"
